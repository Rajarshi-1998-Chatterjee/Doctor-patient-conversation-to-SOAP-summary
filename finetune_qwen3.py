"""Fine-tune Qwen/Qwen3-1.7B on medical SOAP note generation from train.json and validation.json."""
...
train_path = "./Data/Notes_to_soap_summ/train.json"
validation_path = "./Data/Notes_to_soap_summ/validation.json"
OUTPUT_MODEL_DIRECTORY = "./qwen3_1.7B_soap_finetuned"
...
def format_prompt_for_training(example: Dict) -> str:
    ...
    # Format as instruction-following conversation for Qwen3
...
def setup_model_and_tokenizer():
    """Load and prepare model and tokenizer with LoRA for efficient fine-tuning."""
...
def generate_summary(model, tokenizer, dialogue: str) -> str:
    """Generate SOAP note for a given dialogue."""
    input_text = f"""<|im_start|>system
You are an expert medical assistant that generates SOAP notes from dialogues.<|im_end|>
<|im_start|>user
Create a Medical SOAP note summary from the following dialogue:
...
def validate_model(model, tokenizer, validation_data: List[Dict], num_samples: int = 5):
    ...
    for i, example in enumerate(validation_data[:num_samples]):
        ...
        generated_soap = generate_summary(model, tokenizer, dialogue)
        logger.info(f"\nGenerated SOAP:\n{generated_soap}")
        logger.info(f"{'─'*80}\n")


def finetune_granite():
    """Main function to fine-tune Qwen3-1.7B with LoRA."""
    logger.info("="*80)
    logger.info("Qwen3-1.7B LoRA Fine-tuning for Medical SOAP Note Generation")
    logger.info("="*80)
    ...
    logger.info("\nAll done! You can now use the fine-tuned model for medical SOAP note generation.")
