"""Test microsoft/MediPhi-Clinical model on SOAP summary generation from medical dialogues."""
...
model_path = "microsoft/MediPhi-Clinical"  # Path to MediPhi-Clinical model
test_path = "/Users/rajarshichatterjee/Desktop/NLP_FOR_HEALTHCARE/Voice Case Summarizer/Data/Notes_to_soap_summ/test.json"  # Path to test data
results_path = "/Users/rajarshichatterjee/Desktop/NLP_FOR_HEALTHCARE/Voice Case Summarizer/mediphi_test_results.json"  # Path to save results
...
def load_model_and_tokenizer(model_path: str):
    """Load MediPhi-Clinical model and tokenizer."""
...
def generate_soap_summary(model, tokenizer, dialogue: str, prompt: str = None) -> str:
    """Generate SOAP summary for a given dialogue using MediPhi-Clinical."""
    if prompt is None:
    ...
    # Format prompt for MediPhi-Clinical model
    input_text = f"""<|system|>
You are an expert medical professor assisting in the creation of medically accurate SOAP summaries. Please ensure the response follows the structured format: S:, O:, A:, P: without using markdown or special formatting.</s>
<|user|>
{prompt}
...
"""
    ...
    if "<|assistant|>" in generated_text:
        soap_summary = generated_text.split("<|assistant|>")[-1]
        soap_summary = soap_summary.replace("</s>", "").strip()
        return soap_summary
    
    # Fallback: remove the input prompt
    generated_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
    if input_text.strip() in generated_text:
        soap_summary = generated_text.replace(input_text.strip(), "").strip()
        return soap_summary
...
def test_model():
    """Main function to test the MediPhi-Clinical model."""
    logger.info("="*80)
    logger.info("MediPhi-Clinical SOAP Summary Generation - Testing Pipeline")
    logger.info("="*80)
    ...
    for i, example in enumerate(tqdm(test_data, desc="Testing")):
        ...
        custom_prompt = example.get("prompt", None)
        
        # Generate prediction
        predicted_soap = generate_soap_summary(model, tokenizer, dialogue, custom_prompt)
    ...
    logger.info(f"Average ground truth SOAP length: {avg_ground_truth_len:.1f} characters")
    logger.info(f"Average predicted SOAP length: {avg_predicted_len:.1f} characters")
